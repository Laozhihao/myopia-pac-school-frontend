...
export const DetailModal: React.FC<API.ModalItemType> = (props) => {
  ...
  useMemo(async () => {
    if (visible) {
      ...
      const { data } = await getScreeningStudentDetailList({screeningPlanStudentId, screeningPlanId});
      ...
      {inspectType.map((item) =>
        detailDataSource[item.dataIndex] ? (
          <div key={item.dataIndex} className={styles.content}>
            ...
            <Table
              dataSource={Array.isArray(detailDataSource[item.dataIndex]) ? detailDataSource[item.dataIndex] : [detailDataSource[item.dataIndex]]}
