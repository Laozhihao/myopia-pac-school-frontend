...
export const AddModal: React.FC<API.ModalItemType & FileCardPropsParams> = (props) => {
  ...
  const [hostPath, setHostPath] = useState(''); // 域名
  ...
  useEffect(() => {
    ...
    const localHostPath = 'https://s-myopia-pac-report.tulab.cn';
    setHostPath(localHostPath);
    setIframeSrc(
  ...
  const onPdf = () => {
    ...
    a.contentWindow.postMessage('打印', hostPath);
  ...
  return (
    ...
    >
      <Spin spinning={loading}>
        <Button type="primary" onClick={onPdf}>
          生成PDF
        </Button>
