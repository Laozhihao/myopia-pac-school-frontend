...
function getViewerConfiguration() {
  return {
    ...
    printPreview: 'false' //是否默认打开打印预览，默认值stirng 'false'
  };
}
...
var PDFViewerApplication = {
  ...
  load: function load(pdfDocument) {
    ...
    pdfDocument.getMetadata().then(function (_ref5) {
      ...
      if (pdfTitle) {
        _this5.setTitle("".concat((contentDispositionFilename && decodeURIComponent(contentDispositionFilename)) || document.title));
      } else if (contentDispositionFilename) {
        _this5.setTitle(contentDispositionFilename && decodeURIComponent(contentDispositionFilename));
      }
...
function webViewerInitialized() {
  ...
  //判断是否默认打开打印预览
  var printPreview =  params.printpreview;
  if(printPreview){
    appConfig.printPreview = printPreview;
  }
  // 判断是否使用自定义字体，若有则挂载到window下，目的是为了统一mac字体采用类黑体，在url传入参数iscustomtypeface为true
  var isCustomTypeFace = params.iscustomtypeface;
  if (typeof window !== 'undefined' && isCustomTypeFace === 'true') {
     window['isCustomTypeFace'] = isCustomTypeFace;
   }
...
function parseQueryString(queryString) {
     // 传入的参数规则为 参数1：xx& 参数……&file=pdf地址，参数file放最后
      // 传入的字符串是已经编码过的，先解码
      var query = decodeURIComponent(queryString);
      //获取pdf路径，并进行编码
      var urlIndex = query.indexOf('file=')+5;
      var pdfUrl = encodeURIComponent(query.slice(urlIndex));
      //获取pdf的url编码后的query
      var paramString = query.slice(0,urlIndex);
      paramString += pdfUrl;
      // 获取params 对接{参数1：value，参数2：value}
...
function () {
  function PDFSidebar(_ref) {
    ...
    this.isOpen = true; //默认为不展开左侧pdf视图导航
  ...
  function PDFHistory(_ref) {
    ...
    this.eventBus.on('pagesloaded', function (evt) {
      _this._isPagesLoaded = !!evt.pagesCount;
       ...
       // 打开打印预览
      if(PDFViewerApplication.appConfig.printPreview === 'true') {
        window.print();
      }
    });
  }
