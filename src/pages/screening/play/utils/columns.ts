import { ASTIGMATISM_LEVEL, EMPTY, GLASSES_TYPE_LIST, HYPEROPIA_LEVEL, MYOPIA_LEVEL } from "@/utils/constant";
 ...
 export const visionColumn = [
  {
    ...
    render: ( text: any, record: any) => {
      ...
      const { lowVision, isAstigmatism, isHyperopia, isMyopia, screeningMyopia, myopiaLevel, hyperopiaLevel, astigmatismLevel } = record;
      ...
      const descStr = descArr.length
        ? `，${descArr.join('、')}`
        : '';

      const result = text === null
        ? EMPTY
        : `${GLASSES_TYPE_LIST[text]}${descStr}`;
      return result;
    },
  }
];